...
declare module "expo-router" {
  ...
  // prettier-ignore
  type StaticRoutes = `/` | `/(tabs)/` | `/(tabs)` | `/(tabs)/two` | `/two` | `/modal` | `/polls/new` | `/(auth)/login` | `/login` | `/(protected)/profile` | `/profile`;
  // prettier-ignore
  type DynamicRoutes<T extends string> = `/polls/${SingleRoutePart<T>}`;
  // prettier-ignore
  type DynamicRouteTemplate = `/polls/[id]`;
